...
* http://qs_launch_permalink[Deploy {partner-product-short-name} into a new VPC^]. This option builds a new AWS environment that consists of a new VPC, public subnets, Application Load Balancer, Spot Elastigroup, and other infrastructure components. It then deploys {partner-product-short-name} into this new VPC with predictive auto scaling and Inteligent Traffic Flow enabled. 
